...
# Detail Sasana
def detail_sasana(request, id):
    sasana = get_object_or_404(Sasana, id_sasana=id)
    return render(request, 'sasana_detail.html', {'sasana': sasana})

# Edit Sasana
def update_sasana(request, id):
    sasana = get_object_or_404(Sasana, id_sasana=id)
    if request.method == 'POST':
        form = SasanaForm(request.POST, instance=sasana)
        if form.is_valid():
            form.save()
            return redirect('list-sasana')
    else:
...
# Detail Peserta
def detail_peserta(request, id):
    peserta = get_object_or_404(Peserta, id_peserta=id)
    return render(request, 'peserta_detail.html', {'peserta': peserta})

# Edit Peserta
def update_peserta(request, id):
    peserta = get_object_or_404(Peserta, id_peserta=id)
...
# Detail Instruktur
def detail_instruktur(request, id):
    instruktur = get_object_or_404(Instruktur, id_instruktur=id)
    return render(request, 'instruktur_detail.html', {'instruktur': instruktur})

# Edit Instruktur
def update_instruktur(request, id):
    instruktur = get_object_or_404(Instruktur, id_instruktur=id)
...
# Delete Instruktur
def delete_instruktur(request, id):
    instruktur = get_object_or_404(Instruktur, id_instruktur=id)
    if request.method == 'POST':
        instruktur.delete()
